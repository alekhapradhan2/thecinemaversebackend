...
const LANG_KEY    = argv.includes("--language") ? argv[argv.indexOf("--language")+1].toLowerCase() : "hindi";
...
const langConfig = LANGUAGES[LANG_KEY] || LANGUAGES.hindi;
...
const BlogSchema = new mongoose.Schema({
  ...
  seoTitle:String,seoDesc:String,language:String,
},{timestamps:true});
...
function buildMoviePrompt(movie) {
  ...
  const genres   = (movie.genre||[]).join(", ") || langConfig.adjective;
  const cast     = (movie.cast||[]).slice(0,8).map(c=>`${c.name}${c.role?" ("+c.role+")":""}`).join(", ") || "notable cast";
  const director = movie.director || `a ${langConfig.adjective} director`;
  ...
  return `Write a high-quality, unique, SEO-optimized article about the ${langConfig.adjective} movie "${movie.title}".
...
- Director: ${director}
- Cast: ${cast}
- Language: ${movie.language||langConfig.adjective}
${runtime}
${songs ? "- Songs: "+songs : ""}
${synopsis}
${rating}
...
- SEO-friendly: naturally include keywords like "${langConfig.adjective} movie", "${langConfig.industry}", "${movie.title}", "${year}"

Structure (write as flowing paragraphs, not labeled sections):
1. Introduction — release year, genre, why this movie matters in ${langConfig.adjective} cinema
...
function buildTop10Prompt(year, movies) {
  ...
  return `Write a high-quality, engaging SEO article: "Top 10 ${langConfig.adjective} Movies of ${year}".
...
- Include an intro paragraph about ${langConfig.adjective} cinema in ${year}
- End with a conclusion about what ${year} meant for ${langConfig.industry}
- Natural, engaging tone — like a film critic's year-end roundup
- Include keywords: "${langConfig.adjective} movies ${year}", "${langConfig.industry} ${year}", "best ${langConfig.adjective} films ${year}"
...
1. SEO Title (60 chars max, include movie name + "${langConfig.adjective} Movie" + year if known)
2. Meta Description (155 chars max, compelling, includes main keywords)
3. Tags (5-8 comma-separated tags like: "${title}, ${langConfig.adjective} Movie, ${langConfig.industry}, ...")
...
{"seoTitle":"...","seoDesc":"...","tags":["..."],"excerpt":"..."}`;
  ...
  try {
    ...
    return {
      seoTitle: `${title} — ${langConfig.adjective} Movie Review & Details | The Cinema Verse`,
      seoDesc:  articleText.slice(0,155),
      tags:     [title, `${langConfig.adjective} Movie`, langConfig.industry, `${langConfig.adjective} Cinema`],
...
async function saveBlogPost({ title, content, meta, movie, category }) {
  ...
  const doc = {
    ...
    language:   movie?.language || langConfig.dbValue,
  };
...
async function main() {
  console.log("═══════════════════════════════════════════════════════");
  console.log("  The Cinema Verse Article Generator");
  console.log("═══════════════════════════════════════════════════════");
  console.log(`Language     : ${langConfig.adjective} (${langConfig.industry})`);
  ...
  const query = { language: { $regex: new RegExp(`^${langConfig.dbValue}$`, "i") } };
  let movies = await Movie.find(query, "title releaseDate genre director producer synopsis posterUrl cast media reviews verdict runtime language").lean();
  ...
  if (!MOVIE_FILTER) {
    ...
    const allMovies = await Movie.find({ verdict: { $ne: "Upcoming" }, language: { $regex: new RegExp(`^${langConfig.dbValue}$`, "i") } }, "title releaseDate verdict director").lean();
    const byYear = {};
    ...
    for (const [year, yMovies] of Object.entries(byYear).sort((a,b)=>b[0]-a[0]).slice(0,5)) {
      ...
      const title = `Top 10 ${langConfig.adjective} Movies of ${year} — Best ${langConfig.industry} Films`;
      ...
      try {
        ...
        const article = await callLocalAI(buildTop10Prompt(year, sorted));
    ...
    const upcoming = await Movie.find({ verdict: "Upcoming", language: { $regex: new RegExp(`^${langConfig.dbValue}$`, "i") } }, "title releaseDate genre director").lean();
    if (upcoming.length >= 3 && !DRY_RUN) {
      const title   = `Upcoming ${langConfig.adjective} Movies — Most Anticipated ${langConfig.industry} Films`;
      const exists  = await Blog.findOne({ title }).lean();
      if (!exists) {
...
- Intro about the state of ${langConfig.adjective} cinema and what fans can look forward to
- End with an encouraging note for ${langConfig.adjective} film lovers
- SEO keywords: "upcoming ${langConfig.adjective.toLowerCase()} movies", "new ${langConfig.adjective.toLowerCase()} film 2025 2026", "${langConfig.industry} upcoming"
